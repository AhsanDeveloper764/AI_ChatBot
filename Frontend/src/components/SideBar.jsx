...
const SideBar = ({ismenuOpen,setIsMenuOpen}) => {
  const {user,chats,setSelectedChats,theme,setTheme,navigate} = useAppContext();
  ...
  return <>
    ...
    max-md:absolute left-0 z-1 ${!ismenuOpen && "max-md:-translate-x-full"} `} >
      ...
      {/* New Chat Btn */}
      <button className='flex justify-center items-center w-full py-2 mt-10 text-white bg-gradient-to-r from-[#A456F7]
      to-[#3D81F6] text-sm rounded-md cursor-pointer'> 
        <span className='mr-2 text-xl'>+</span> New Chat </button>

      {/* Search Conversation */}
      <div className='flex items-center gap-2 p-3 mt-4 border border-gray-400 dark:border-white/20 rounded-md' >
        <img src={assets.search_icon} className='w-4 not-dark:invert' alt="" />
        <input onChange={(e)=>setSearch(e.target.value)} value={search} type="text" name="" id="" placeholder='Search Conversation' 
      ...
      <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'>
        {
          chats.filter((chat) => chat.messages[0] ? chat.messages[0]?.content.toLowerCase().includes(search.toLowerCase()) : 
            chat.name.toLowerCase().includes(search.toLowerCase())).map((chat) => (
              ...
              key={chat._id} className='p-2 px-4 dark:bg-[#57317C]/10 border border-gray-300 
              dark:border-[#80609F]/15 rounded-md cursor-pointer flex justify-between group'>
                ...
                <img src={assets.bin_icon} className='hidden group-hover:block w-4 cursor-pointer 
                not-dark:invert' alt="" />                
              </div>
          ))
        }
        ...
        dark:border-white/15 rounded-md'>
          ...
          <label className='relative inline-flex cursor-pointer'>
            <input type="checkbox" className='sr-only peer' onChange={()=>setTheme(theme === "dark" ? "light" : "dark")} 
            checked={theme === "dark"} />
        ...
        <img src={assets.close_icon} onClick={()=>setIsMenuOpen(false)}
