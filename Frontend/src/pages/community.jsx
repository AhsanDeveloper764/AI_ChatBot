          ...
          <div className='flex flex-wrap max-sm:justify-center gap-5' >
            {images.map((item,index)=>{
              ...
              border border-gray-200 dark:border-purple-700 shadow-sm hover:shadow-md transition-shadow duration-300' >
                <img src={item.imageUrl} alt="" className='w-full h-40 md:h-50 2xl:h-62 object-cover group-hover:scale-105
                transition-transform duration-300 ease-in-out'/>
                <p className='absolute bottom-0 right-0 text-xs bg-black/50 backdrop-blur
                text-white px-4 py-1 rouded-tl-xl opacity-0 group-hover:opacity-100 transition
                duration-300'>Created By {item.userName}</p>
              </a>
              )
            })}
          </div>
